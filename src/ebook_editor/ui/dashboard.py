...
class DashboardView:
    ...
    def render(self) -> None:
        ...
        if self.container is None:
            self.container = ui.column().classes("w-full h-full p-8 max-w-6xl mx-auto")
        else:
            self.container.clear()
        with self.container:
            # Header
            with ui.row().classes("w-full items-center justify-between pb-6 border-b border-gray-700"):
            ...
            with ui.row().classes("w-full mt-6 gap-8"):
                ...
                with ui.column().classes("flex-1 gap-4"):
                    ui.label("Workspace Library").classes("text-lg font-semibold text-gray-200")
